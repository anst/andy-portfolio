...
$streak->route('/', function($streak) {
	...
	foreach($posts as $post) {
		...
		$title = substr(explode("\n", file_get_contents($post))[0],0, 1)==='!'?substr(explode("\n", file_get_contents($post))[0],2):substr(explode("\n", file_get_contents($post))[0],1);
		if(substr(explode("\n", file_get_contents($post))[0],0, 1)==='!') {
			continue;
		}
...
$streak->route('/<int>/<int>/<int>/<string>', function($streak, $year,$month,$day,$slug) {
	...
	if($contents === FALSE) {
		...
		$date = $year.'-'.$month.'-'.$day;
		$title = substr(explode("\n", $contents)[0],0, 1)==='!'?substr(explode("\n", $contents)[0],2):substr(explode("\n", $contents)[0],1);
